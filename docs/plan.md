...
4. README covering the config reference, the `extra_specs` schema, the DinD-mode matrix, and NAS platform notes, plus example configs.
   - **Trust-topology note**: GARM itself and this provider are the trusted parties in this system and legitimately need host Docker access to do their job — that is by design, not a gap. The provider itself can never run as a container: GARM `exec`s the configured `provider_executable` as a filesystem path directly, and an OCI image reference cannot be that path, so there is no hook that would make GARM run `docker run …` on the provider's behalf. The GHCR provider image (item 1 above) is a distribution vehicle only — operators extract the static binary onto the host via `docker create`+`docker cp`, or `COPY --from=` it into their own GARM image at build time — and the binary always runs as a host process GARM execs directly, inheriting host Docker socket access from GARM's own process context there. The runner and DinD sidecar containers the provider creates for jobs never get that socket, in any mode (ADR-001). The README states this distinction explicitly and warns operators against ever co-mounting the host socket into a runner container themselves (e.g. via a custom `podTemplate`-equivalent override) — doing so would silently defeat this project's core isolation guarantee regardless of what the provider's own code does correctly.
